...
const WeeklyPriceReduction: React.FC<WeeklyPriceReductionProps> = ({ data }) => {
    ...
    useEffect(() => {
        ...
        setWeeklyPriceReductionData(processedData);
    }, [data]);

    // Filter data by selected year
    const filteredData = selectedYear === 'all' 
        ? weeklyPriceReductionData 
        : weeklyPriceReductionData.map(item => ({
            week: item.week,
    ...
    return (
        <div className="lg:col-span-2 bg-white/70 backdrop-blur-sm border-white/20 rounded-lg p-6 hover:shadow-xl transition-all duration-300">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
                <div>
                    <h3 className="text-slate-800 text-lg font-semibold">Weekly Price Reductions</h3>
                    <p className="text-slate-600 text-sm">Price reductions by week and year</p>
                </div>
            ...
            {filteredData.length > 0 && (
                <div className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-red-50 p-3 rounded-lg">
                        <div className="text-sm text-red-600 font-medium">Total Reductions</div>
                    ...
                    <div className="bg-orange-50 p-3 rounded-lg">
                        <div className="text-sm text-orange-600 font-medium">Average Weekly</div>
                    ...
                    <div className="bg-blue-50 p-3 rounded-lg">
                        <div className="text-sm text-blue-600 font-medium">Latest Week</div>
            ...
            {filteredData.length > 0 && (
                <div className="mt-4 p-4 bg-slate-50 rounded-lg">
                    <h4 className="text-sm font-semibold text-slate-700 mb-2">Price Reduction Analysis</h4>
                    <div className="text-xs text-slate-600 space-y-1">
                        {(() => {
                            const totalReductions = filteredData.reduce((sum, item) => {
                                if (selectedYear === 'all') {
                                    return sum + years.reduce((yearSum, year) => yearSum + (item[year] || 0), 0);
                                }
                                return sum + (item[selectedYear] || 0);
                            ...
                            const avgWeekly = Math.round(totalReductions / filteredData.length);
                            const latestWeek = filteredData[filteredData.length - 1];
                            ...
                            const isHigh = latestValue > avgWeekly * 1.2;
                            const isLow = latestValue < avgWeekly * 0.8;
                            
                            return (
                                <>
                                    <p>• Total price reductions: {totalReductions.toLocaleString()}</p>
                                    <p>• Average weekly reductions: {avgWeekly.toLocaleString()}</p>
                                    <p>• Latest week: {latestValue.toLocaleString()} ({trend} average)</p>
                                    {isHigh && <p>• High activity: Latest week significantly above average</p>}
                                    {isLow && <p>• Low activity: Latest week below average</p>}
                                    {!isHigh && !isLow && <p>• Normal activity: Latest week near average</p>}
...
export default WeeklyPriceReduction
